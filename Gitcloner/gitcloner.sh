...
init_progress() {
    ...
    # Only show progress bar if terminal is interactive and not in quiet mode
    if [ -t 2 ] && [ "$VERBOSITY" -ge 2 ]; then
        log_message "DEBUG" "Progress tracking initialized for ${total} repositories"
    fi
}

# Show overall progress bar for repository list
show_overall_progress() {
    local current="$1"
    local total="$2"
    local repo_name="$3"
    
    # Only show if we have a terminal and not in quiet mode
    if [ ! -t 2 ] || [ "$VERBOSITY" -lt 2 ] || [ "$total" -eq 0 ]; then
        return
    fi
    
    PROGRESS_CURRENT="$current"
    PROGRESS_REPO_NAME="$repo_name"
    ...
    if [ "$elapsed" -gt 3600 ]; then
        ...
        elapsed_str=$(printf "00:%02d" "$elapsed")
    fi
    
    # Print progress bar
    printf "\r\033[KProgress: %s %3d%% (%d/%d) | Elapsed: %s | ETA: %s | %s" \
        "$bar" "$percent" "$current" "$total" "$elapsed_str" "$eta" "$repo_name"
    
    # If this is the last item, add a newline
...
# Show per-repository git clone progress (parses git output)
show_git_progress() {
    local git_output="$1"
    local stage="$2"
    
    # Only show if we have a terminal and in verbose mode
    if [ ! -t 2 ] || [ "$VERBOSITY" -lt 3 ]; then
        return
    fi
    
    # Parse different git progress formats
    local progress_text=""
    
    # Git clone receiving objects
    if [[ "$git_output" =~ Receiving\ objects:\ +([0-9]+)% ]]; then
        local percent="${BASH_REMATCH[1]}"
        progress_text="Receiving objects: ${percent}%"
    
    # Git clone resolving deltas
    elif [[ "$git_output" =~ Resolving\ deltas:\ +([0-9]+)% ]]; then
        local percent="${BASH_REMATCH[1]}"
        progress_text="Resolving deltas: ${percent}%"
    
    # Git pull updating files
    elif [[ "$git_output" =~ Updating\ files:\ +([0-9]+)% ]]; then
        local percent="${BASH_REMATCH[1]}"
        progress_text="Updating files: ${percent}%"
    
    # Git pull general progress with counts
    elif [[ "$git_output" =~ ([0-9]+)/([0-9]+) ]]; then
        ...
        progress_text="${stage}: ${percent}% (${current}/${total})"
    fi
    
    # If we found progress info, show it
...
show_completion() {
    ...
    if [ "$elapsed" -gt 3600 ]; then
        ...
        elapsed_str=$(printf "%d seconds" "$elapsed")
    fi
    
    printf "\n\033[1;32m✓\033[0m Completed \033[1m${success_count}/${total}\033[0m repositories in ${elapsed_str}\n"
}

# Handle git command with progress parsing
run_git_with_progress() {
    local command="$1"
    local description="$2"
    local repo_name="$3"
    
    # Only show progress if we have a terminal and in verbose mode
    if [ ! -t 2 ] || [ "$VERBOSITY" -lt 3 ]; then
        eval "$command"
        return $?
    fi
    
    log_message "DEBUG" "Executing git command with progress tracking: $description"
    
    # Create named pipe for progress output
    local progress_pipe=$(mktemp -u)
    mkfifo "$progress_pipe"
    
    # Start the git command
    eval "$command" > >(tee -a "$LOG_FILE") 2> >(tee -a "$LOG_FILE" > "$progress_pipe") &
    local git_pid=$!
    
    # Monitor progress output
    while kill -0 "$git_pid" 2>/dev/null && read -r line < "$progress_pipe" 2>/dev/null; do
        show_git_progress "$line" "$description"
    done &
    local monitor_pid=$!
...
update_progress() {
    ...
    local total="$2"
    local repo_name="$3"
    local status="$4"  # "start", "success", "fail", "skip"
    
    # Update progress bar
    show_overall_progress "$current" "$total" "$repo_name"
    ...
    case "$status" in
        ...
        "fail")
            ...
            ;;
        "skip")
            log_message "WARNING" "Skipped repository: ${repo_name}"
            ;;
    esac
}
...
# Custom clone directory (override OS-specific default)
# CUSTOM_CLONE_DIR="${HOME}/my_repositories"

# Git user configuration (optional)
...
clone_repository() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        clear_progress
        if [ -t 2 ] && [ "$VERBOSITY" -ge 3 ]; then
            printf "  Cloning: %s\n" "$repo_url"
        fi
        
        # Use progress-aware git clone
        if run_git_with_progress "git clone \"$repo_url\" \"$target_dir\"" "Cloning" "$repo_name"; then
            clone_success=true
            break
        else
...
update_existing_repo() {
    ...
    # Try main branch first with progress
    if run_git_with_progress "(cd \"$repo_dir\" && git pull origin main)" "Updating (main)" "$(basename "$repo_dir")"; then
        log_message "SUCCESS" "Repository updated successfully from main branch"
        return 0
    fi
    
    # Try master branch if main failed with progress
    if run_git_with_progress "(cd \"$repo_dir\" && git pull origin master)" "Updating (master)" "$(basename "$repo_dir")"; then
...
process_all_repositories() {
    ...
    for repo_info in "${REPOSITORIES[@]}"; do
        ...
        # Show progress for this repository
        update_progress "$current_index" "$total_count" "$repo_name" "start"
        ...
        case $result in
            0)  # Success
                ...
                update_progress "$current_index" "$total_count" "$repo_name" "success"
                ;;
            1)  # Failure
                fail_count=$((fail_count + 1))
                update_progress "$current_index" "$total_count" "$repo_name" "fail"
                ;;
            2)  # Skipped
                skip_count=$((skip_count + 1))
                update_progress "$current_index" "$total_count" "$repo_name" "skip"
                ;;
        esac
    ...
    show_completion "$success_count" "$total_count"
...
generate_final_summary() {
    ...
    # Clear progress line before showing summary
    clear_progress
