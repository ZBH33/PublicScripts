...
source ./functions/message_utils.sh
source ./functions/os_detect.sh
source ./functions/check_root_no.sh
source ./functions/check_ssh_keygen.sh
...
main() {
    # Enable ERR trap ONLY if message_utils.sh has been sourced.
    if command -v enable_error_trap >/dev/null 2>&1; then
        set -e
        enable_error_trap
    fi

    # Run detect OS function    
    detect_os   

    # Run check root function
    check_root_no

    # Run check OpenSSH-server
    check_ssh_keygen
}
